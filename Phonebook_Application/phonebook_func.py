...
def center_window (self, w, h): #pass in the tkinter frame(master) refrence and the w and h
    #gets the users screen width and height
    screen_width = self.master.winfo_screenwidth()
    screen_height = self.master.winfo_screenheight()
    
    #calculates x and y coords to paint the app centered on users screen
...
# catch if the user click on the windows 'X' to ensure they want to close 
def ask_quit(self):
...
def onSelect(self, event):
    ...
    with conn:
        ...
        #this returns a tuble and it can slice it into 4 parts using data[] druning the iteration
        for data in varBody:
...
def addToList(self):
    ...
    #Normalize data to keep it consistent in the database
    ...
    if (len(var_fname) > 0 ) and (len(var_lname) > 0 ) and (len(var_phone) > 0 ) and (len(var_email) > 0 ):
        ...
        with conn:
            cursor = conn.cursor()
            #checking for databse for existance of the full name, if so will get alert user and disregard request
...
def onDelete(self):
    ...
    with conn:
        ...
        #checks count to ensure that it is not the last record in
        # the database... 
